...
RC6.8 起应用包提供 SOCKS 代理所需的轻量模块；使用 SOCKS 代理下载 Whisper 等
模型时无需再向系统 Python 安装 `httpx[socks]`，也不必重装已就绪的 AI 运行环境。
RC6.9 在代理连接被拒绝时会尝试其他连接方式，自动选择的官方模型源连接失败时也会尝试
内置国内镜像。如果两条线路都失败，请检查本机代理是否运行，或切换网络后重试；
程序不会把网络中断当成模型文件损坏，也不会删除已有模型缓存。

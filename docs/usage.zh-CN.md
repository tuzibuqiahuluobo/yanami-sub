...
本文面向 `v0.1.0-rc.6.post4`（RC6.4）。字幕处理由固定版本的
...
1. 在 [Yanami Sub Releases](https://github.com/tuzibuqiahuluobo/yanami-sub/releases)
   下载 `Yanami-Sub-0.1.0-rc.6.post4-Setup.exe` 和对应的 `.sha256` 文件。
2. 在 PowerShell 中运行以下命令校验安装包：

   ```powershell
   Get-FileHash .\Yanami-Sub-0.1.0-rc.6.post4-Setup.exe -Algorithm SHA256
   ```

3. 将输出与 `.sha256` 文件中的值比较，完全一致后再运行安装器。
4. 如果电脑上装有 FineSub Desktop，请先从托盘菜单退出旧程序。RC6.4 会沿用原安装记录、
...
卸载器，所以 RC4 仍需要手动运行 RC6.4 Setup 覆盖安装；RC5 起已修复该保护逻辑，RC5.2
起每个桌面 Release 都会发布在线更新清单和签名，RC5 至 RC6.3 可通过
“应用更新”检查新版本。

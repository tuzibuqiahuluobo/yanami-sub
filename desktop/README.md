...
运行环境是 `uv` 在安装目录里建的一个虚拟环境，它需要一个现成的 CPython 3.12 作为**基底**。
程序按这个顺序找：

1. 你在“资源 → 运行诊断 → Python 解释器”里**手动指定**的解释器；
2. `python3.12` / `python`（PATH）；
3. `py` 启动器注册的解释器（`py -0p`，**包括没勾选“Add to PATH”的安装**）；
4. `%LOCALAPPDATA%\Programs\Python` 等常见安装位置。

找到一个就用它建环境，只下载 AI 依赖；一个都找不到时，`uv` 会自己下载一份私有 Python
（2.8 GB）。后者在部分 Windows 机器上会失败——`uv` 需要建一个 junction，而某些
用户配置目录不允许遍历装入点，报 `os error 448`。此时不要反复点重试（每次都走同一条路），
请按上面的顺序补一个 3.12，或在诊断里手动指定：

- 安装 python.org 的 64 位 3.12 并勾选 **Add python.exe to PATH**，或
- 已有 3.12 时把它的目录加进用户 `Path`（放在最前面，避免被
  `%LOCALAPPDATA%\Microsoft\WindowsApps` 的应用执行别名抢先），或
- 在“运行诊断 → Python 解释器”点“指定解释器”直接选 `python.exe`——**保存后需要重启应用**
  （基底解释器在任何窗口出现之前就已经确定了）。

诊断里会列出**每个被检查过的候选以及拒绝理由**（版本不符、不是解释器、路径不存在），
所以“我明明装了 Python”这种情况能看出是哪一个被拒了。

> 另一个与下载手感有关的环境变量：`FINESUB_RESOURCE_SEARCH_PATHS`。安装任何资源前，
> 程序会扫描本机磁盘寻找同名压缩包以便复用（上限 45 秒）。默认扫描下载/桌面/文档和大文件盘；
> 盘多或盘大时可以把扫描面收窄到指定目录，用 `;` 分隔。
...
RC3 是首次改名迁移版。旧 FineSub Desktop 与 RC4 必须手动运行 Yanami Sub 安装器；RC5/RC5.1
可以通过“应用更新”升级到 RC5.2，也可以手动运行 Setup。RC5 起已保护卸载器文件，RC5.2
起每个 Release 都会附带在线更新清单和签名。
